...
describe "_::yield" {
    ...
    describe "when passed a block" {
        ...
        proc acceptsBlockAndReturnsExecuted { block } {
        ...
        describe "calling return -code return inside a block" {
            proc helperProc {} {
                puts [acceptsBlockAndReturnsExecuted {{} {
...
describe "_::eachSlice" {
    it "yields the given list in slices of the given size" {
        set result [list]
        _::eachSlice [list 1 2 3 4 5 6 7 8] 3 {{ slice } {
            upvar result result
...
describe "_::each" {
    ...
    it "allows non-local returns" {
        expect [eachWithNonLocalReturn] to equal "non-local return"
    }
}
...
describe "_::reduceRight" {
    it "can perform right folds" {
        expect [_::reduceRight {"foo" "bar" "baz"} {{ memo str } {
            return "${memo}${str}"
        }} ""] to equal "bazbarfoo"
    }
